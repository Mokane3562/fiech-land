...
public class ResourceActionMenu extends JFrame {
	...
	/**
	 * Performs a resource action with a specific chance of success. Happiness gained is equal to the specified score multiplier multiplied by the Game.players resource attribute. Costs one action point.
	 * 
	...
	public void performResourceAction(int mult, double percentChance){
		...
		if (Game.player.getActionPoints() > 0) {
			...
			if (random <= percentChance) {
				 ...
				 */
				Game.player.setHappiness(Game.player.getHappiness() + Game.player.getResources() * mult);
